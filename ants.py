...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_position = 0
        while current_place.entrance is not None and current_place.name is not 'Hive':
            if self.min_range <= current_position <= self.max_range:
...
# END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor-=amount
        if self.armor<=0 and self.name!='Imposter Queen':
            bees_win()
        else:
            self.place.remove_insect(self)
            self.death_callback()
